node {
    ...
    stage('Run Aqua Scan'){
        ...
        try{
            sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v '+env.WORKSPACE+':/reports registry.devopslab.co:5000/aquasec/scanner-cli:2.6.0 --local -image image_to_scan:tag --host ${AQUA_SERVER_URL} --user ${AQUA_USER} --password ${AQUA_PASS} --htmlfile /reports/aqua-scan.html'
        }catch(e){
            buildResult = 'failure'
